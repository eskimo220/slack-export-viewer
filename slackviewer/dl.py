...
class slackdl:
    ...
    def __init__(self, t):
        self.client = WebClient(token=t)

    def getUsers(self, prev=None):
        result = self.client.users_list(cursor=prev)

        if result["response_metadata"]["next_cursor"]:
            return result["members"] + self.getUsers(
                result["response_metadata"]["next_cursor"])
        else:
            return result["members"]

    def getChannels(self, type, prev=None):
        result = self.client.conversations_list(cursor=prev, types=type)

        if result["response_metadata"]["next_cursor"]:
            return result["channels"] + self.getChannels(
                type, result["response_metadata"]["next_cursor"])
        else:
            return result["channels"]

    def getMsgs(self, channel_id, oldest=0, prev=None):
        result = self.client.conversations_history(cursor=prev,
                                                   oldest=oldest,
                                                   channel=channel_id)

        if result["has_more"] and result["response_metadata"]["next_cursor"]:
            return result["messages"] + self.getMsgs(
    ...
    def writeTofile():
        ...
        with open(os.path.join(path, "channels.json"), mode='w') as f:
            ...
            for i in p:
                ...
                for k, g in groupby(
                        reversed(getMsgs(i["id"])),
                        lambda x: datetime.fromtimestamp(float(x[
                            "ts"])).strftime('%Y-%m-%d.json')):
                    with open(os.path.join(path, i["name"], k),
                              mode='w') as ff:
